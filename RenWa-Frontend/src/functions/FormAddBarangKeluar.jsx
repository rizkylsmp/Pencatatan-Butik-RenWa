...
const FormAddBarangKeluar = ({ show, onClose, onSave }) => {
  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        ...
        const karyawanUsers = response.data.filter(
          (user) => user.role === "karyawan"
        );
        setUsers(karyawanUsers);
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-40 text-color-1 text-nowrap">
      ...
      <div className="bg-color-3 p-5 rounded shadow-lg w-4/5 max-h-[80vh] overflow-y-auto">
        <h2 className="font-bold text-xl mb-4">Tambah Data Barang Keluar</h2>
...
export default FormAddBarangKeluar;
