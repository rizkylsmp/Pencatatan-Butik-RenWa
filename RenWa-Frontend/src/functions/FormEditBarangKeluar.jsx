...
const FormEditBarangKeluar = ({ show, onClose, onSave, data }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.patch(
        `${process.env.REACT_APP_API_BASE_URL}/barang-keluar/${data.uuid}`,
        formDataToSend,
        {
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center text-color-1 text-nowrap">
      ...
      <div className="bg-color-3 p-5 rounded shadow-lg w-4/5 max-h-[80vh] overflow-y-auto">
        <h2 className="font-bold text-xl mb-4">Edit Data Barang Keluar</h2>
...
export default FormEditBarangKeluar;
