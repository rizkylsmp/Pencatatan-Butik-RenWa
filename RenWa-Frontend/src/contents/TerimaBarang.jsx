...
import FormAddData from "../functions/FormAddTerimaBarang.jsx";
import FormEditData from "../functions/FormEditTerimaBarang.jsx";
...
const TerimaBarang = () => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/terima-barang`
      );
  ...
  const deleteData = async (dataId) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/terima-barang/${dataId}`
      );
      getData();
    } catch (error) {
  ...
  const filename = `data_terima_barang_${currentDate
    .toLocaleDateString()
    .replace(/\//g, "-")}.csv`;
  ...
  return (
    <div className="p-5 h-fit bg-color-5 rounded shadow-xl md:text-base text-sm text-color-3">
      ...
      <h2 className="md:text-2xl text-xl font-bold pb-3">Data Terima Barang</h2>
