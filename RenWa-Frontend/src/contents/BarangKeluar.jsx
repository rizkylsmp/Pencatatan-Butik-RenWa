...
import FormAddData from "../functions/FormAddBarangKeluar.jsx";
import FormEditData from "../functions/FormEditBarangKeluar.jsx";
...
const BarangKeluar = () => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/barang-keluar`
      );
  ...
  const deleteData = async (dataId) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/barang-keluar/${dataId}`
      );
      getData();
    } catch (error) {
  ...
  const filename = `data_barang_keluar_${currentDate
    .toLocaleDateString()
    .replace(/\//g, "-")}.csv`;
  ...
  return (
    <div className="p-5 bg-color-5 rounded shadow-xl md:text-base text-sm text-color-3">
      {isLoading && (
      ...
      <h2 className="text-2xl font-bold pb-3">Data Barang Keluar</h2>
        ...
        <table className="bg-white min-w-full text-nowrap text-color-5">
          <thead>
            <tr className="border-b-2 border-color-5 text-left">
              ...
              <th className="px-6 py-3" onClick={() => requestSort("pengirim")}>
                <div className="flex gap-2 items-center cursor-pointer">
                  Kepada {getIcon("pengirim")}
                </div>
              </th>
          ...
          <tbody>
            {currentItems.map((item, index) => (
              <tr key={item.uuid} className="border-b border-color-5">
                ...
                <td className="px-6 py-4">{item.kepada}</td>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col gap-1 text-center">
                    ...
                    <button
                      ...
                      className="bg-color-6 hover:scale-105 py-1 px-2 text-color-5"
                    >
                      Delete
                    </button>
                  </div>
