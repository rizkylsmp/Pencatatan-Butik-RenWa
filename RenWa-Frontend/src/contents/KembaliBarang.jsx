...
import FormAddData from "../functions/FormAddKembaliBarang.jsx";
import FormEditData from "../functions/FormEditKembaliBarang.jsx";
...
const KembaliBarang = () => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/kembali-barang`
      );
  ...
  const deleteData = async (dataId) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/kembali-barang/${dataId}`
      );
      getData();
    } catch (error) {
  ...
  const filename = `data_kembali_barang_${currentDate
    .toLocaleDateString()
    .replace(/\//g, "-")}.csv`;
  ...
  return (
    <div className="p-5 h-fit bg-color-5 rounded shadow-xl md:text-base text-sm text-color-3">
      ...
      <h2 className="md:text-2xl text-xl font-bold pb-3">
        Data Kembali Barang
      </h2>
